...
$utility		= new Utility();
	...
	for($i = 0; $i < $dataSize; $i++){
        ...
        if ($information[0] == 'bank_ref_no') {
            $TXNID = $information[1]; // transection id
        }
		...
		if($information[0]== 'payment_mode')	$paymentMode = $information[1]; 
...
$cusId		= $utility->returnSess('userid', 0);
	...
	if($order_status==="Success"){
        ...
        $_SESSION['paypalTranId'] = $TXNID;

        $orderStatus   = "Ordered";
        $paymentStatus = "Completed";
    
    
        $order = $Gporder->gpOrderById($ORDERID);
        $updated = $order[0]['date'];
        $updateOrder = $Gporder->successPayment($ORDERID, $TXNID, $paymentMode, $paymentStatus, $orderStatus);
        $updateTrack = $Gporder->ccavTrackingInsert($ORDERID, $tracking_id);
        // print_r($order);

...
if(isset($_POST['btnAddTown']))
{
	...
	{
		//add town
		$t_id = $lc->addTown($txtCountriesId, $txtProvinceId, $txtCountyId, $txtName, $txtDesc);
            ...
            <div id="admin-body">
                ...
                <div id="options-area">
                	<div class="add-new-option">
                    	<a href="<?php echo $_SERVER['PHP_SELF']."?action=add_town".$link."#addt"; ?>">
				   Add Town </a>
                    </div>
                </div>
                <!-- eof Options -->
				...
				<?php 
					...
					$noOfTown = array_slice($noOfTown, $start, $limit);
					
					foreach($noOfTown as $k)
				    {
					    //$k = $pageArray[$pageNumber][$j];
						
						$townDetail 	= $lc->getTownDataByTownId($k);	
											
						$bgColor 		= $utility->getRowColor($x);
				?>
					<tr align="left" class="blackLarge" <?php $utility->printRowColor($bgColor);?>>
					  <td><?php echo $x++; ?></td>
					  <td><?php echo $townDetail[3]; ?></td>
					  <td>
					  	<?php 
							if($townDetail[6] != '')
							{
								echo "<div style='padding:0px 10px 10px 2px;'>".substr($townDetail[6],0,100)."</div>";
							}
							else
							{
					  ...
					  <td>
					  <?php echo $utility->getSingleValueByKey($townDetail[1], "province_id", "province_name", "province", ''); ?>
					  </td>
					  <td  class="">
					  <?php echo $dateUtil->printDate($townDetail[8]); ?>
					  </td>
					  <td>
                        ...
                        <span>Please note that all the <span class="required">*</span> marked fileds are required</span>
                         <form action="<?php echo $_SERVER['PHP_SELF']."?action=add_town".$page->getLink();?>" 
                          method="post" enctype="multipart/form-data">
                              ...
                              <label>&nbsp;</label>
                                    <input name="btnAddTown" type="submit" class="button-add" id="btnAddTown" value="add" />					

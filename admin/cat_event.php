...
if(isset($_POST['btnCreateCat'])) 
{
	...
	$duplicate = $error->duplicateCat($parent_id, $cat_name,0, 'event_categories');
	...
	{
		//add the category
		$catId = $category->createCategory($parent_id, $cat_name, $txtURL, $txtBrief, $txtPageTitle, $txtDesc, 'event_categories');
		...
		if($_FILES['fileCatImg']['name'] != '')
		{
			...
			$uImg->imgUpdResize($_FILES['fileCatImg'], 'STATCAT', $newName, 
								'../images/category/', 200, 200, $catId,
					 			'categories_image', 'categories_id', 'event_categories');	   
		}
...
<title><?php echo COMPANY_S; ?> - Event Category</title>
            ...
            <div id="admin-body">
                ...
                <?php 
				  ...
				  if(isset($_GET['catId']))
				  {
					 ...
					 .$parentPath.$category->categoryPath($myCatId,'YES','event_categories').
					 "<span>";			 	
				  } 
                ...
                <div id="data-column">
                    ...
                    $parentCat = $category->displayCategory($catId,'event_categories');
                    ...
                    $rootCat   = $category->displayRoot('event_categories');
                    if(count($rootCat) == 0)
                    {
                    ?>
                    ...
                    <?php 
                        ...
                        foreach($parentCat as $k)
                        {
                            $catDetail 	= $category->categoryData($k,'event_categories');
                    ...
                    ?>
                        <tr align="left" <?php $utility->printRowColor($bgColor);?>>
                          ...
                          <td>
                          <?php 
                          if(count($category->displayCategory($k,'event_categories')) > 0)
                          {
                            echo "<a href='".$_SERVER['PHP_SELF']."?catId=".$k."'>Subcategories(".count($category->displayCategory($k,'event_categories')).")</a>";
                          }
                          else
                          {
                            echo "Subcategories(".count($category->displayCategory($k,'event_categories')).")";
                          }
                          ?>
                          ...
                          <td>
                          <?php 
						  $total = count($category->getEventList($k, 0,'all')) + count($category->getEventList($k, 0,'only'));
						  if( $total > 0)
						  {
							echo "<a href='#'>Event (".$total.")</a>";
						  }
						  else
						  {
							echo "Event (".count($category->getEventList($k, 0,'all')).")";
						  }
						  //echo $cls->getProductList($k, 'all');.$k
						  ?>
						  </td>
                          <td><?php echo $dateUtil->printDate($catDetail[4]); ?></td>
                          <td>
                          <?php 
                          if($k != 1)
                          {
                          ...
                          onClick="MM_openBrWindow('cat_event_edit.php?action=edit_cat&amp;id=<?php echo $k; ?>','CatEdit','scrollbars=yes,width=700,height=600')">
                          edit					  
                          </a> ]
                          
                          [ 
                          <a href="javascript:void(0)" onClick="MM_openBrWindow('cat_event_del.php?action=delete_cat&id=<?php echo $k; ?>','CatDelete','scrollbars=yes,width=400,height=350')">
                          delete</a> 
                          ]
                          <br /><br />
                          <?php 
                          ...
                          [ 
                          <a href="<?php echo $_SERVER['PHP_SELF']."?action=add_cat&parent_id=".$k."#addCat"; ?>" >
                          add sub cat
                          </a> ]
                ...
                <div class="pagination-bottom">
                    <div class="upper-block">Total Number of Static Content Category: <?php echo count($rootCat);?></div>
                ...
                <div class="webform-area">
					...
					<span>Please note that all the <span class="required">*</span> marked fileds are required</span>
					  ...
					  <form action="<?php $_SERVER['PHP_SELF']?>" method="post" enctype="multipart/form-data">
							...
							<label>Parent Category	 <span class="orangeLetter">*</span></label>		
							<select name="txtParentId" class=" textBoxA" id="txtParentId">
							  ...
							  {
								$category->categoryDropDown(0,0,$_GET['parent_id'],'ADD',0,'event_categories');
							  }
							  elseif(isset($_SESSION['parent_id']))
							  {
								$category->categoryDropDown(0,0,$_SESSION['parent_id'],'ADD',0,'event_categories');
							  }
							  else
							  {
								$category->categoryDropDown(0,0,0,'ADD',0,'event_categories');
							  }
							  ?>
							</select>
                            <div class="cl"></div>

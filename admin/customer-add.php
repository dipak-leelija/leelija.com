...
if(isset($_POST['btnAddCus'])) 
{
	...
	$sess_arr	= array( 'txtEmail', 'txtPassword','txtfirstName', 'txtLastName',  'radioRt',  'radioAcc', 'txtDesc', 
						 'txtCompanyName','txtProfession', 'intOrder','txtDiscountOffered', 'txtAddress1', 'txtTown',  
...
/*	if(!isset($_POST['radioDesg'])  )
	{ 	
		$error->showErrorTA($action, $id, $id_var, $url, ERU115, $typeM, $anchor);
	}
	...
	elseif($radioRt == '')
	{
		$error->showErrorTA($action, $id, $id_var, $url, ERU128, $typeM, $anchor); 
	}
		else*/if( ($txtUserName == '') ||(preg_match("^ER^",$duplicateId)) )		
	{
		$error->showErrorTA($action, $id, $id_var, $url, ERU103, $typeM, $anchor);
	}
	elseif( ($txtEmail == '') ||(preg_match("^ER^",$invalidEmail)) )		
	{
	...
	{	
		...
		//add customer
		$customerId = $customer->addCustomer( 1, '', $txtUserName, $txtEmail, $txtPassword,  $txtfirstName,  $txtLastName, 'na',
											 'a', '', $txtDesc, $txtCompanyName, 'Y',  $txtProfession, $intOrder, $verificationNo,
											 $radioAcc, $txtDiscountOffered); 

		//add the address
		$customer->addCusAddress( $customerId,  $txtAddress1,  '',  '',  $txtTown,  $txtProvince, $txtPostalCode, $txtCountriesId,
								 $txtPhone1, '', $intFax, $intMobile);
		...
		//forward
		$uMesg->showSuccessT('success', 0, '', 'customer.php', SUCUST205, 'SUCCESS');
            ...
            <div class="webform-area">
                ...
                ?>
                    ...
                    <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
                        ...
                        <?php /*?><label>Customer Type<span class="required">*</span></label>
                        <input type="radio" name="radioRt" id="radioRt" class="radio"  value="1" title="1"
                        <?php echo $utility->checkSessStr('radioRt','1', '');?>/>
                        <label for="radioRt">Member</label>
                        
                        
                        <input type="radio" name="radioRt" id="radioRt" class="radio" value="2" title="2" 	
                        <?php echo $utility->checkSessStr('radioRt','2', '');?> />
                        <label for="radioRt">Volunteer </label>
                        
                        
                        <input type="radio" name="radioRt" id="radioRt" class="radio" value="3" title="3" 	
                        <?php echo $utility->checkSessStr('radioRt','3', '');?> />
                        <label for="radioRt">Sponser </label>
                       ...
                       value="<?php $utility->printSess('txtTown'); ?>" size="30" />
                       <div class="cl"></div>	
                 
                       <label> Province<span class="orangeLetter"> *</span></label>
                       <input name="txtProvince"  class="text_box_large_amba" id="txtProvince" 
                       value="<?php $utility->printSess('txtProvince'); ?>" size="25" />
                       <div class="cl"></div>
                       ...
                       <div class="cl"></div>
                        ...
                        <label>Select Country<span class="orangeLetter">*</span></label>
                        <select name="txtCountriesId" class=" textBoxA" id="txtCountriesId">
                        <option value="0">-- Select --</option>
                        <?php
                        if(isset($_SESSION['txtCountriesId']))
                        {
                            $utility->populateDropDown($_SESSION['txtCountriesId'], 'countries_id',
                                                       'countries_name', 'countries');
                        }
                        elseif(isset($_GET['txtCountriesId']) && ((int)$_GET['txtCountriesId'] > 0))
                        {
                            $utility->populateDropDown($_GET['txtCountriesId'], 'countries_id',
                                                       'countries_name', 'countries');
                        }
                        else
                        {
                     ...
                     <?php /*?><h3>Newsletter</h3> 
                       ...
                       <input name="btnAddCus"  id="btnAddCus" type="submit" class="button-add" value="create" /><!--button-cancel-->

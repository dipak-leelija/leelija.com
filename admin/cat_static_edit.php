...
if(isset($_GET['id']))
{
	$numOrder	= $uNum->genSortOrderNum('Y', $_GET['id'], 'categories_id', 1, 'static_categories');
}
else
{
	$numOrder	= $uNum->genSortOrderNum('Y', 0, 'categories_id', 1, 'static_categories');
}
...
if(isset($_POST['btnEditCat']))
{
	...
	$duplicate = $error->duplicateEntry($cat_name, 'categories_name', 'static_categories', 'YES', $cat_id, 'categories_id');
	...
	{
		//edit category
		$catId = $category->editCategory($parent_id, $cat_name, $txtURL, $txtBrief, $txtPageTitle, $txtDesc, $numOrder, $cat_id, 
										 'static_categories');
		...
		{
			$utility->deleteFile($cat_id, 'categories_id' ,'../images/category/', 'categories_image', 'static_categories');
		...
		if($_FILES['fileCatImg']['name'] != '')
		{
			...
			$uImg->imgUpdResize($_FILES['fileCatImg'],'STATCAT',$newName,'../images/category/', 
					 200, 200, $cat_id,'categories_image', 'categories_id', 'static_categories');
...
<div class="popup-form">
    ...
    method="post" enctype="multipart/form-data">
        ...
        <select name="txtParentId" class=" textBoxA" id="txtParentId">
          ...
          <?php
            $category->categoryDropDown(0, 0 ,$catDetail[2],'edit',0,'static_categories');
          ?>
        </select>
        <div class="cl"></div>
        ...
        value="<?php echo $catDetail[0];?>" size="25" /><div class="cl"></div>

        <label>Link or URL</td></label>
